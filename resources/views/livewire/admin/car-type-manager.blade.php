        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-slate-200">
            <div class="p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    <h2 class="text-xl font-bold text-slate-800">Car Type Management</h2>
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="relative" x-data="{ uploading: false, progress: 0 }"
                            ...
                            <input type="file" wire:model="excelFile" class="hidden" id="type-excel-upload"
                                accept=".xlsx,.xls,.csv" wire:change="import">
                            <label for="type-excel-upload"
                ...
                <div class="mb-6 relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search car types..."
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($carType->image_path)
                                            <img src="/uploads/{{ $carType->image_path }}" alt="{{ $carType->name }}"
                                                class="h-10 w-10 object-contain rounded bg-slate-50 border border-slate-200">
                                        @else
                                            <div
                                                class="h-10 w-10 rounded bg-slate-100 flex items-center justify-center text-slate-400 text-xs font-bold border border-slate-200">
                                                N/A
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-semibold text-slate-900">{{ $carType->name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800">
                                            {{ $carType->brand->name ?? 'N/A' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="px-2.5 py-0.5 inline-flex text-xs font-bold rounded-full {{ $carType->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div x-data="{ open: false }" class="relative inline-block text-left">
                                            ...
                                            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                                ...
                                                style="display: none;">
                                                <div class="py-1">
                                                    <button wire:click="edit({{ $carType->id }}); open = false;"
                                                    ...
                                                    <button wire:click="delete({{ $carType->id }})"
                                                        wire:confirm="Are you sure you want to delete this car type?"
                                                        @click="open = false;"
                ...
                <div
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-slate-100">
                        <h3 class="text-lg leading-6 font-bold text-slate-900" id="modal-title">
                            {{ $isEditing ? 'Edit Car Type' : 'Create Car Type' }}
                        </h3>
                        <div class="mt-6 space-y-5">
                            ...
                            <div>
                                ...
                                @error('brandId') <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('image') <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                                @enderror
